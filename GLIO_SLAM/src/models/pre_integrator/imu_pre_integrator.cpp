...
#include "GLIO_SLAM/models/pre_integrator/imu_pre_integrator.hpp"

#include "GLIO_SLAM/global_defination/global_defination.h"

#include "glog/logging.h"
#define PI 3.1415926
#define w_ie_ 7.292115e-5
namespace GLIO_SLAM {
...
bool IMUPreIntegrator::Update1(const IMUData &imu_data,const VelocityData &velocity_data,const GNSSData &gnss_data) {
    if ( imu_data_buff_.front().time < imu_data.time ) {
        // set buffer:
        imu_data_buff_.push_back(imu_data);
        velocity_data_buff_.push_back(velocity_data);
 ...
 * @return void
 */
void IMUPreIntegrator::UpdateState1(void) {
    static double T = 0.0;
    ...
    //addnew
    const VelocityData &curr_velocity_data=velocity_data_buff_.front();
    const GNSSData &curr_gnss_data=gnss_data_buff_.front();
    // get time delta:
    T = curr_imu_data.time - prev_imu_data.time;

    Eigen::Vector3d vel_a,a_w_M,vel_a1,gnss_LBH_pose;
    gnss_LBH_pose(0)=curr_gnss_data.latitude;
    gnss_LBH_pose(1)=curr_gnss_data.longitude;
    gnss_LBH_pose(2)=curr_gnss_data.altitude;
    
    
    Eigen::Vector3d w_n_en1,w_n_en2;
    Eigen::Quaterniond q(curr_imu_data.orientation.w, 
    curr_imu_data.orientation.x, curr_imu_data.orientation.y, curr_imu_data.orientation.z);
    Eigen::Matrix3d R_nb=q.matrix().cast<double>();
    vel_a(0)=curr_velocity_data.linear_velocity.x;
    vel_a(1)=curr_velocity_data.linear_velocity.y;
    vel_a(2)=curr_velocity_data.linear_velocity.z;

    // vel_a=Eigen::Vector3d::zero();
    // w_n_ie=Eigen::Vector3d::zero();
    // w_n_en=Eigen::Vector3d::zero();
    // w_n_en1=Sophus::Vector3d::zero();
    // a_w_M=Eigen::Matrix3d::zero();
    //double PI=3.1415926;
    
    double  R_N=6378254/sqrt(1-(2*(1/298.3)-(1/298.3)*(1/298.3))*sin(gnss_LBH_pose(1)*PI/180)*sin(gnss_LBH_pose(1)*PI/180));
    double  R_M=R_N*(1-(2*(1/298.3)-(1/298.3)*(1/298.3)))/(1-(2*(1/298.3)-(1/298.3)*(1/298.3))*sin(gnss_LBH_pose(1)*PI/180)*sin(gnss_LBH_pose(1)*PI/180));
    Eigen::Vector3d w_n_ie(0,w_ie_*cos(gnss_LBH_pose(1)*PI/180),w_ie_*sin(gnss_LBH_pose(1)*PI/180));
    Eigen::Vector3d w_n_en(-vel_a1(1)/(R_M+gnss_LBH_pose(2)),vel_a1(0)/(R_N+gnss_LBH_pose(2)),vel_a1(0)/(R_N+gnss_LBH_pose(2))*tan(gnss_LBH_pose(1)*PI/180));
    w_n_en2=w_n_ie+ w_n_en;
    w_n_en1=2*w_n_ie+ w_n_en;

    
    Eigen::Matrix3d  Wf=Sophus::SO3d::hat(w_n_en1);
    Eigen::Vector3d a_w= R_nb.transpose()*w_n_en2;
    Eigen::Vector3d w_w= R_nb.transpose()*Wf*vel_a;



    // get measurements:
   const Eigen::Vector3d prev_w(
        prev_imu_data.angular_velocity.x - state.b_g_i_.x()-a_w(0),
        prev_imu_data.angular_velocity.y - state.b_g_i_.y()-a_w(1),
        prev_imu_data.angular_velocity.z - state.b_g_i_.z()-a_w(2)
    );
    const Eigen::Vector3d curr_w(
        curr_imu_data.angular_velocity.x - state.b_g_i_.x()-a_w(0),
        curr_imu_data.angular_velocity.y - state.b_g_i_.y()-a_w(1),
        curr_imu_data.angular_velocity.z - state.b_g_i_.z()-a_w(2)
    );

    const Eigen::Vector3d prev_a(
        prev_imu_data.linear_acceleration.x - state.b_a_i_.x()-w_w(0),
        prev_imu_data.linear_acceleration.y - state.b_a_i_.y()-w_w(1),
        prev_imu_data.linear_acceleration.z - state.b_a_i_.z()-w_w(2)
    );
    const Eigen::Vector3d curr_a(
        curr_imu_data.linear_acceleration.x - state.b_a_i_.x()-w_w(0),
        curr_imu_data.linear_acceleration.y - state.b_a_i_.y()-w_w(1),
        curr_imu_data.linear_acceleration.z - state.b_a_i_.z()-w_w(2)
    );
