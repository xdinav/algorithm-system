...
bool LIOBackEnd::InitDataPath(const YAML::Node& config_node) {
    ...
    if (!FileManager::CreateDirectory(data_path + "/data_interface"))
        return false;

    key_frames_path_ = data_path + "/data_interface/key_frames";
    scan_context_path_ = data_path + "/data_interface/scan_context";
    trajectory_path_ = data_path + "/data_interface/trajectory";
...
bool LIOBackEnd::UpdateIMUPreIntegration(const IMUData &imu_data,const VelocityData &velocity_data,const GNSSData &gnss_data) {
    if ( !graph_optimizer_config_.use_imu_pre_integration || nullptr == imu_pre_integrator_ptr_ )
        return false;

    if (
        !imu_pre_integrator_ptr_->IsInited() ||
        imu_pre_integrator_ptr_->Update1(imu_data,velocity_data,gnss_data)
    ) {
        return true;
    }
