...
#include "GLIO_SLAM/models/registration/ndt_registration.hpp"
#include "GLIO_SLAM/models/cloud_filter/voxel_filter.hpp"
...
bool FrontEnd::Update(const CloudData& cloud_data, Eigen::Matrix4f& cloud_pose) {
    ...
    if (fabs(last_key_frame_pose(0,3) - current_frame_.pose(0,3)) +
        ...
        std::cout << "---------------UpdateWithNewFrame before--------------" << std::endl;
        UpdateWithNewFrame(current_frame_);
        std::cout << "---------------UpdateWithNewFrame after--------------" << std::endl;
...
bool FrontEnd::UpdateWithNewFrame(const Frame& new_key_frame) {
    ...
    std::cout << local_map_frames_.size() << std::endl;

    // scan-to-map matching:
    // set target as local map:
    if (local_map_frames_.size() < 10) {
        std::cout << "SetInputTarget before" << std::endl;
        registration_ptr_->SetInputTarget(local_map_ptr_);
        std::cout << "SetInputTarget after" << std::endl;
    } else {
